...
// Note: if the monitor freeze, check trek emco parameters for cap cal
...
float Gain_T = 0.02996; // 33.25 is the gain of the non-inverting op amp, 543 is the DFT to voltage conversion
float Gain_R = 0.00717; // 1.01 is the attenuation of the 2nd filter, 0.20145 is the attenuation of the inverting opamp
// Constants to compute the actuator's capacitance
float Rm = 31100; // Need measurement
float Rs = 1; //  Need measurement  2MEG
float Cf = 220 * pow(10, -12);
float Rvs = 70000.742; // trek 
float Rvp = 6435000000; // trek
float C = 4022 * pow(10, -12); // Emco C80 3113
...
float coef_arr[2] = {0.25, -40}; // This is for 660pF? cap baseline
...
void setup() {
  ...
  adc->startContinuous(A13, ADC_1);// This is for EMCO PSU (comment this if using Trek)
...
void loop() {
  ...
  V_Rm  = sqrt(Re_R * Re_R + Im_R * Im_R) * Gain_R*2.543;
  V_Tot = sqrt(Re_T * Re_T + Im_T * Im_T) * Gain_T * 3.752;
  theta = atan2(Im_R, Re_R) + (8.0 + 180) * pi / 180 - atan2(Im_T, Re_T);
...
//  Serial.println(V_Tot);
  ...
  int data_out = ((Ca - 0) / (10000 - 0) * 4095);
  analogWrite(A21, data_out);
