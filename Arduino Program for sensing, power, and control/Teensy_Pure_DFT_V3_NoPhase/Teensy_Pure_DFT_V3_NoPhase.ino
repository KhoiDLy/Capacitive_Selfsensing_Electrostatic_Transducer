...
float Gain_R = 0.00717/2*1.025; // 1.07 is the attenuation of the 2nd filter, 0.20145 is the attenuation of the inverting opamp
// Constants to compute the actuator's capacitance
float Rm = 31100; // Need measurement
float Rs = 115; //  Need measurement  2MEG
float Cf = 220 * pow(10, -12);
float Rvs = 1000; // trek 
float Rvp = 1; // trek
float C = 1 * pow(10, -12); // Emco C80 3113
...
//    Im_T = Im_T + Buffer_T[i] * sinArray[i];
  ...
  V_Rm  = sqrt(Re_R * Re_R + Im_R * Im_R) * Gain_R*1000;
  
  int data_out = ((V_Rm - 80) / (800 - 80) * 4095);
