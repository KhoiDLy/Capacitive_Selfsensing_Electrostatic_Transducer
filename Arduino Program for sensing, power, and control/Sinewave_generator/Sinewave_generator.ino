/*
  The following settings are for:
            PDB0_DACINT0 = 2000 = 10 Hz, 1000 = 20Hz,4000 = 5Hz, 8000 = 2.5Hz, 16000 = 1.25Hz, 32000 = 625mHz
     with   BUFFER_SIZE 3000 ; sinetable[i] = 2048+(int)(2048*sin(((float)i)*6.28318530717958647692/((float)BUFFER_SIZE)));

 Default settings: BUFFER_SIZE 480 ; sinetable[i] = 2048+(int)(2048*sin(((float)i)*20*6.28318530717958647692/((float)BUFFER_SIZE))); PDB0_DACINT0 = 19 => 125kHz
...
void setup() {
  ...
  for(int i=0; i<BUFFER_SIZE; i++) {
    sinetable[i] = 2250+(int)(1250*sin(((float)i)*6.28318530717958647692/((float)BUFFER_SIZE)));
  }
  // initialise the DAC
  ...
  for (int16_t i=0; i<2048; i+=1) {
    *(int16_t *)&(DAC0_DAT0L) = i;
    delayMicroseconds(250); // this function may be broken
  }
  ...
  PDB0_DACINT0 = (uint16_t)(16000); // we won't subdivide the clock...
